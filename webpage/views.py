...
def change_profile():
    if request.method == 'POST':
        ...
        if not oldPword:
            flash(
                'Please enter the old password can\'t update a blank line', category='error')
        elif not name:
            flash('Name field have no value', category='error')
        elif not email:
            flash('Email field have no value', category='error')
        elif not newPword:
            flash('Please enter a password can\'t update a blank line',
                  category='error')
        else:
            if not check_password_hash(current_user.pword, oldPword):
                ...
                User.query.filter_by(name=current_user.name).update(
                    dict(name=name, pword=generate_password_hash(newPword), email=email))
                db.session.commit()
    ...
    if note.notes == current_user.id:
        if request.method == 'POST':
            ...
            if not title:
                ...
                Note.query.filter_by(id=id).update(
